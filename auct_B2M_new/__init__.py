                ...
                for t in trds:
                    ...
                    if trd_sgn == ord_sgn:
                        Deals.create(sessioncode=player.session.code, period=ts.CurrentPeriod,
                                     ...
                                     instr=int(data['instr']), price=t['p'], quantity=float(t['q']), ord_init=t['ord1'],
                                     ord_other=t['ord2'], time=tm)
                    else:
                        Deals.create(sessioncode=player.session.code, period=ts.CurrentPeriod,
                                     trial=player.round_number, pl_other=t['trd'], pl_init=t['trd2'], bs=ord_sgn,
                                     instr=int(data['instr']), price=t['p'], quantity=float(t['q']),
                                     ord_other=t['ord1'], ord_init=t['ord2'], time=tm)
                # отправка обновлния 
